...
for boy in boys:
    for girl in girls:
        ...
        if boy_interests.intersection(girl_interests) \
                and abs(boy['age'] - girl['age']) <= 6 \
                and boy['name'] not in girl['ex'] and girl['name'] not in boy['ex']:
            # we need the abs difference between ages in case boy's age is smaller that girl's age
            # and we receive a negative number
            print("{} ({}) и {} ({}) ; общи интереси: {} "
                  .format(girl['name'], girl['age'], boy['name'], boy['age'],
                          ', '.join(boy_interests & girl_interests)))
